...
这个 fork 上的拉取请求现在能得到一个真实、快速、始终能给出结论的验证结果(`static` + `unit`,均运行在标准 runner 上),而不再排队等待永远不会到位的容量。issue 与拉取请求事件不再由 `issue-lifecycle`/`issue-policy` 产生自动失败的检查,master push 与夜间运行也不再由 `e2e.yml` 产生自动失败的检查。`container-publish.yml` 依旧照旧在 master push 与 `dsh-v*` 标签上构建并发布 `ghcr.io/opscalehub/deepseek-harness`。
